...
def test_parse_data_escaped_strings():
    """
    Verifies that escape sequences in .asciz directives are correctly
    ...
    .data
    complex_esc:
        .asciz "Path:\\Win\tName:\"Foo\"\nNext"
    """

    parse_data(stub, source_code)

    expected_value = 'Path:\\Win\tName:"Foo"\nNext'

    assert len(stub.calls) == 1
    call_method, call_value, call_name = stub.calls[0]

    assert call_method == "allocate_data"
    assert call_name == "complex_esc"
    assert call_value == expected_value

    # Validation of lengths:
    # Path: (5) + \ (1) + Win (3) + \t (1) + Name: (5) + " (1) + Foo (3) + " (1) + \n (1) + Next (4)
    # Total characters = 25
    assert len(call_value) == 25
